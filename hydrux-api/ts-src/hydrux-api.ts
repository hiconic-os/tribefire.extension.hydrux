...
export function hxApplication(): IHxApplication {
    return ($tf as any).hydrux.application;
}
...
export interface IHxHostSettings {
    /* Hydrux servlet guarantees these do not end with a slash. */
    servicesUrl(): string;
    webSocketUrl(): string
    /* DomainId where the HxApplication MD should be resolved */
    domainId(): string;
...
export interface IHxComponentBinder {
    /* Hint: you can simply reference a function with proper signature as a ComponentFactory.
     *  Example: bindComponent(myModel.MyCustomComponent, createMyCustomComponent) */
...
export type IHxComponentFactory<T extends hxM.HxComponent> = (denotation: T, context: IHxComponentCreationContext) => Promise<IHxComponent>;
export type IHxViewFactory<T extends hxM.HxView> = (denotation: T, context: IHxComponentCreationContext) => Promise<IHxView>;
export type IHxRequestDialogFactory<T extends hxM.HxView> = (denotation: T, context: IHxComponentCreationContext) => Promise<IHxRequestDialog<any, any>>;
...
type IHxServiceProcessorFunction<SR extends ServiceRequest, R> = (context: service.ServiceRequestContext, request: SR) => Promise<Maybe<R>>;
...
export interface IHxServiceProcessorBinder {
    ...
    bind<T extends ServiceRequest>(
        requestType: EntityType<T>, processor: IHxServiceProcessor<T, any>): IHxServiceProcessorBinding;
    /** Binds a service processsor for given ServiceRequest type */
    bindFunction<T extends ServiceRequest>(
        requestType: EntityType<T>, processor: IHxServiceProcessorFunction<T, any>): IHxServiceProcessorBinding;

    /** Binds a service processsor for given ServiceRequest type and serviceId. */
    bindDispatching<T extends DispatchableRequest>(
        requestType: EntityType<T>, serviceId: string, processor: IHxServiceProcessor<T, any>): IHxServiceProcessorBinding;
    /** Binds a service processsor for given ServiceRequest type and serviceId. */
    bindDispatchingFunction<T extends DispatchableRequest>(
        requestType: EntityType<T>, serviceId: string, processor: IHxServiceProcessorFunction<T, any>): IHxServiceProcessorBinding;
    ...
    bindDialogProcessor<T extends ServiceRequest>(
        requestType: EntityType<T>, requestDialog: hxM.HxRequestDialog): IHxServiceProcessorBinding;
}
...
// there is a static access point for the HxApplication
...
export interface IHxApplication {
    ...
    // This is async because resolving a scope in general also involes eagerly resolving it's controllers, which need to be awaited
...
export interface IHxDomainSupplier extends IHxComponent { getDomain(): Promise<Maybe<string>>; }
export interface IHxSessionHolder extends IHxComponent { getSession(): PersistenceGmSession; }
export interface IHxSessionFactory extends IHxComponent { newSession(): PersistenceGmSession; }
